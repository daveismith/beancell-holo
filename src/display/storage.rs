...
use crate::display::{DISPLAY_NAME_MAX_LEN, DISPLAY_PASS_MAX_LEN};
use crate::shared_flash::{SharedFlash, SharedMapStorage};

const DISPLAY_SSID_KEY: u8 = 4;
const DISPLAY_PASS_KEY: u8 = 5;
const DISPLAY_RESERVED_SECTORS: u32 = 2;
const SSID_VALUE_LEN: usize = DISPLAY_NAME_MAX_LEN + 1;
const PASS_VALUE_LEN: usize = DISPLAY_PASS_MAX_LEN + 1;
...
pub struct DisplayCredentialStorage {
    map: SharedMapStorage,
}

impl DisplayCredentialStorage {
    pub const fn new(flash: &'static SharedFlash) -> Self {
        Self {
            map: SharedMapStorage::new(flash, DISPLAY_RESERVED_SECTORS),
        }
    }
    ...
    pub async fn save_credentials(&self, ssid: &str, passphrase: &str) -> Result<(), StorageError> {
        ...
        self.map
            .store_value(DISPLAY_SSID_KEY, &ssid_data)
            .await
            .map_err(|_| StorageError::Flash)?;

        self.map
            .store_value(DISPLAY_PASS_KEY, &pass_data)
            .await
            .map_err(|_| StorageError::Flash)?;
    ...
    pub async fn load_credentials(
        &self,
    ) -> Result<Option<(String<DISPLAY_NAME_MAX_LEN>, String<DISPLAY_PASS_MAX_LEN>)>, StorageError>
    {
        let ssid_data = self
            .map
            .fetch_value::<[u8; SSID_VALUE_LEN]>(DISPLAY_SSID_KEY)
            .await
            .map_err(|_| StorageError::Flash)?;

        let pass_data = self
            .map
            .fetch_value::<[u8; PASS_VALUE_LEN]>(DISPLAY_PASS_KEY)
            .await
            .map_err(|_| StorageError::Flash)?;
        ...
        match (ssid_data, pass_data) {
            (Some(ssid_bytes), Some(pass_bytes)) => {
                let ssid =
                    decode_len_prefixed::<DISPLAY_NAME_MAX_LEN, SSID_VALUE_LEN>(&ssid_bytes)?;
                let pass =
                    decode_len_prefixed::<DISPLAY_PASS_MAX_LEN, PASS_VALUE_LEN>(&pass_bytes)?;
                Ok(Some((ssid, pass)))
            }
            _ => Ok(None),
        }
    ...
    pub async fn clear_credentials(&self) -> Result<(), StorageError> {
        self.map
            .remove_key(DISPLAY_SSID_KEY)
            .await
            .map_err(|_| StorageError::Flash)?;

        self.map
            .remove_key(DISPLAY_PASS_KEY)
            .await
            .map_err(|_| StorageError::Flash)?;
